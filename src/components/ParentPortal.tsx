        ...
        <div className="p-8 lg:p-12 max-w-7xl mx-auto h-full overflow-y-auto pb-40 font-sans">
            <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 animate-in slide-in-from-top-4 duration-500">
                ...
                <div className="flex items-center gap-4">
                  <button className="p-4 bg-white border border-slate-100 rounded-3xl text-slate-400 hover:text-indigo-600 transition-all shadow-sm" aria-label="Notificaciones">
                    <Bell size={24}/>
                  </button>
                        ...
                        <div key={s.id} onClick={() => onSwitchStudent(idx)} className={`bento-card group p-6 rounded-[40px] border-2 cursor-pointer transition-all ${isActive ? 'bg-white border-indigo-600 shadow-2xl shadow-indigo-100' : 'bg-white/40 border-transparent hover:border-slate-200'}`}>
                            <div className="flex items-center gap-5">
                                <div className="relative">
                                  <img src={s.photo} alt={`Foto de ${s.name?.split(' ')[0]}`} className="w-16 h-16 rounded-[22px] object-cover border-2 border-white shadow-lg" />
               ...
               <div className="lg:col-span-2 bg-white rounded-[56px] p-12 border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden bento-card">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row gap-10">
                      <div className="shrink-0 flex flex-col items-center">
                          <img src={student.photo} alt={`Foto de ${student.name}`} className="w-40 h-40 rounded-[48px] object-cover border-8 border-slate-50 shadow-2xl animate-float" />
            ...
            {isLinking && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-6">
                    <div role="dialog" aria-modal="true" aria-label="Vincular estudiante" className="bg-white rounded-[64px] shadow-2xl w-full max-w-xl p-16 relative animate-in zoom-in duration-300">
                        <button onClick={() => setIsLinking(false)} className="absolute top-12 right-12 text-slate-300 hover:text-slate-800 transition-all" aria-label="Cerrar vinculación"><X size={32}/></button>
                        {linkStep === 'form' ? (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                                ...
                                <div className="space-y-10 my-12">
                                    <div className="space-y-3">
                                      <label htmlFor="link-fullname" className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-2">Nombre Completo</label>
                                      <input id="link-fullname" type="text" value={linkForm.fullName} onChange={e => setLinkForm({...linkForm, fullName: e.target.value})} placeholder="Ej. Ana García" className="w-full p-8 bg-slate-50 border-none rounded-[32px] font-black text-xl text-slate-700 outline-none focus:ring-4 focus:ring-indigo-100 transition-all shadow-inner"/>
                                    </div>
                                    <div className="space-y-3">
                                      <label htmlFor="link-schoolkey" className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-2">Clave Colegio</label>
                            ...
                            <div className="animate-in zoom-in duration-500 text-center">
                                ...
                                <div className="bg-slate-50 p-12 rounded-[56px] mb-12 flex flex-col items-center border border-slate-100">
                                    <img src={foundStudent?.photo} alt={`Foto de ${foundStudent?.name}`} className="w-40 h-40 rounded-[48px] object-cover mb-8 border-8 border-white shadow-2xl animate-float" />
